  ...
  where
    portType =
      ...
      case mu of
        ...
        Nothing ->
          if isBiSignalIn hwType then "/*sigPort*/ inout wire" else def <+> "/*sigPort*/ wire"
...
decl :: Declaration -> VerilogM (Maybe Doc) -- Generates /most/ signals inside of modules, but not ports nor map_in and map_out
decl (NetDecl' noteM id_ tyE iEM) = do -- NetDecl' (Maybe Comment) Identifier HWType (Maybe Expr)/*initial value*/
  us <- use usages
  let u = lookupUsage id_ us
  Just A.<$> maybe id addNote noteM' (addAttrs attrs (usageFileDoc u tyE <+> tyDec tyE))
  where
    noteM' = Just (TextS.pack "generated by decl")
...
expr_ x (Identifier id_ (Just (Indexed (Annotated _ (CustomSP _id dataRepr _size args),dcI,fI)))) =
  expr_ x (Identifier id_ (Just (Indexed (CustomSP _id dataRepr _size args,dcI,fI))))
...
expr_ x (Identifier d_ (Just (Indexed (Annotated _ (CustomProduct _id dataRepr _size _maybeFieldNames tys), dcI, fI))))
  | DataRepr' _typ _size [cRepr] <- dataRepr
  , ConstrRepr' _cName _pos _mask _val anns <- cRepr = expr_ x (Identifier d_ (Just (Indexed (CustomProduct _id dataRepr _size _maybeFieldNames tys, dcI, fI))))
...
expr_ x (Identifier id_ (Just (Indexed (Annotated _ (Signed w),y,z))))  =
  expr_ x (Identifier id_ (Just (Indexed ((Signed w),y,z))))
...
-- See [Note] integer projection
expr_ x (Identifier id_ (Just (Indexed (Annotated _ (Unsigned w),y,z)))) =
  expr_ x (Identifier id_ (Just (Indexed ((Unsigned w),y,z))))
...
expr_ b (Identifier x (Just (Indexed (Annotated _ ty@(BitVector _),y,0)))) =
  expr_ b (Identifier x (Just (Indexed (ty,y,0))))
...
expr_ b (Identifier id_ (Just (Indexed (Annotated _ (BitVector w),x,1)))) =
  expr_ b (Identifier id_ (Just (Indexed ((BitVector w),x,1))))
...
expr_ b (DataCon x (DC (Annotated _ (Void {}), -1)) [e]) =
  expr_ b (DataCon x (DC (Void {}, -1)) [e])
expr_ b (DataCon _ (DC (Void {}, -1)) [e]) = expr_ b e

expr_ b (DataCon (Annotated _ ty@(Vector 0 _)) y z) =
  expr_ b (DataCon ty y z)
expr_ _ (DataCon ty@(Vector 0 _) _ _) = verilogTypeErrValue ty

expr_ b (DataCon (Annotated _ (Vector 1 x)) y [e]) =
  expr_ b (DataCon (Vector 1 x) y [e])
expr_ _ (DataCon (Vector 1 _) _ [e]) = expr_ False e

expr_ b e@(DataCon (Annotated _ (Vector x y)) z es@[_,_]) =
  expr_ b (DataCon (Vector x y) z es)
expr_ _ e@(DataCon (Vector _ _) _ es@[_,_]) =
    listBraces $ mapM (expr_ False) $ fromMaybe es $ vectorChain e

expr_ b (DataCon (Annotated _ (MemBlob n m)) x [n0, m0, y, runs, z, ends])
  ...
  , es <- unpackNats n m (B8.pack runs0) (B8.pack ends0) = expr_ b (DataCon (MemBlob n m) x [n0, m0, y, runs, z, ends])
...
expr_ _ (DataCon (Annotated _ (RTree 0 _)) _ [e]) = expr_ False e
expr_ _ (DataCon (RTree 0 _) _ [e]) = expr_ False e

expr_ _ e@(DataCon (Annotated _ (RTree _ _)) _ es@[_,_]) =
    listBraces $ mapM (expr_ False) $ fromMaybe es $ rtreeChain e
expr_ _ e@(DataCon (RTree _ _) _ es@[_,_]) =
    listBraces $ mapM (expr_ False) $ fromMaybe es $ rtreeChain e

expr_ b (DataCon sp@(SP {}) (DC (Annotated _ (BitVector x),y)) es) =
  expr_ b (DataCon sp (DC (BitVector x,y)) es)
...
expr_ b (DataCon (Annotated _ ty@(SP _ args)) (DC (y,i)) es) =
  expr_ b (DataCon ty (DC (y,i)) es)
...
expr_ b (DataCon (Annotated _ (Sum x y)) (DC (z,i)) []) =
  expr_ b (DataCon (Sum x y) (DC (z,i)) [])
expr_ _ (DataCon ty@(Sum _ _) (DC (_,i)) []) = int (typeSize ty) <> "'d" <> int i

expr_ b (DataCon (Annotated _ (CustomSum x y z tys)) (DC (p,i)) []) =
  expr_ b (DataCon (CustomSum x y z tys) (DC (p,i)) [])
...
expr_ b (DataCon (Annotated _ (CustomSP _name dataRepr _size constrs)) (DC (x,constrNr)) es) =
  expr_ b (DataCon (CustomSP _name dataRepr _size constrs) (DC (x,constrNr)) es)
...
expr_ x (DataCon (Annotated _ (CustomProduct y dataRepr _size _labels tys)) z es) |
  DataRepr' _typ _size [cRepr] <- dataRepr = expr_ x (DataCon (CustomProduct y dataRepr _size _labels tys) z es)
expr_ _ (DataCon (CustomProduct _ dataRepr _size _labels tys) _ es) |
  DataRepr' _typ _size [cRepr] <- dataRepr =
  customReprDataCon dataRepr cRepr (zip (map snd tys) es)

expr_ _ (DataCon (Annotated _ (Product {})) _ es) = listBraces (mapM (expr_ False) es)
expr_ _ (DataCon (Product {}) _ es) = listBraces (mapM (expr_ False) es)

expr_ _ (DataCon (Annotated _ (Enable _)) _ [e]) =
  expr_ False e
...
expr_ _ (DataTag (Annotated _ Bool) (Left id_)) = pretty id_ <> brackets (int 0)
expr_ _ (DataTag Bool (Left id_)) = pretty id_ <> brackets (int 0)

expr_ _ (DataTag (Annotated _ Bool) (Right id_)) = do
  iw <- Ap (use intWidth)
...
expr_ _ (DataTag (Annotated _ (Sum _ _)) (Left id_))     = "$unsigned" <> parens (pretty id_)
expr_ _ (DataTag (Sum _ _) (Left id_))     = "$unsigned" <> parens (pretty id_)

expr_ _ (DataTag (Annotated _ (Sum _ _)) (Right id_))    = "$unsigned" <> parens (pretty id_)
expr_ _ (DataTag (Sum _ _) (Right id_))    = "$unsigned" <> parens (pretty id_)

expr_ _ (DataTag (Annotated _ (Product {})) (Right _))  = do
  iw <- Ap (use intWidth)
  int iw <> "'sd0"
...
expr_ _ (DataTag (Annotated _ hty@(SP _ _)) (Right id_)) = "$unsigned" <> parens
                                               (pretty id_ <> brackets
...
expr_ _ (DataTag (Annotated _ (Vector 0 _)) (Right _)) = do
  iw <- Ap $ use intWidth
  int iw <> "'sd0"
...
expr_ _ (DataTag (Annotated _ (Vector _ _)) (Right _)) = do
  iw <- Ap $ use intWidth
  int iw <> "'sd1"
...
expr_ _ (DataTag (Annotated _ (RTree 0 _)) (Right _)) = do
  iw <- Ap $ use intWidth
  int iw <> "'sd0"
...
expr_ _ (DataTag (Annotated _ (RTree _ _)) (Right _)) = do
  iw <- Ap $ use intWidth
  int iw <> "'sd1"
